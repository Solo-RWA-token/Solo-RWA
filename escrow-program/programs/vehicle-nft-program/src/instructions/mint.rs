...
#[derive(Accounts)]
#[instruction(vin_hash: [u8; 32], model: String, color: String, delivery_est: String)]
...
pub fn mint_vehicle_nft(
    ...
    metadata.vin_hash = vin_hash;
    metadata.model = model;
    metadata.color = color;
    metadata.status = VehicleStatus::Reserved as u8;
    metadata.escrow_pda = ctx.accounts.escrow_pda.key();
