...
use anchor_spl::token_interface::{Mint, TokenAccount, TokenInterface, TransferChecked, Burn};
...
pub fn cancel_order(ctx: Context<CancelOrder>) -> Result<()> {
    ...
    if order.funded_amount > 0 {
        ...
        anchor_spl::token_interface::transfer_checked(cpi_ctx, order.funded_amount, ctx.accounts.token_mint.decimals)?;
    }
    ...
    if is_buyer && order.funded_amount > 0 {
        ...
        anchor_spl::token_interface::burn(burn_ctx, order.funded_amount)?;
    }
